		...
		namespace Framework {
			namespace Rendering {
				...
				void CubismRenderContext_SDL3::_RebuildRenderPass(bool isStartFrame)
				{
					...
					//之前对Commander buffer理解不足，离屏渲染不该切换状态而应该分开在两个command buffer中执行
					//后续对这里做点修改
				...
				void CubismRenderContext_SDL3::StartRender()
				{
					...
					//上方是hash形式的检索renderPass的方式
					//按理来说离屏纹理每帧只渲染一次所以应该不需要去维护renderPass map
					//如果未设置_renderTarget _depthStencil说明使用_pass_orig;
